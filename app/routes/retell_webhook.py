...
def _verify_signature(payload_bytes, sig_header):
    """Return True if the Retell HMAC signature is valid."""
...
def _parse_tool_request():
    """Extract args dict and call metadata from Retell's tool POST body."""
...
def tool_get_listing():
    """Return live listing data by listing_id (or best match by name+city)."""
    ...
    if listing_id:
        for lt, getter in [('home', Home.get_by_id),
                           ('experience', Experience.get_by_id),
                           ('party', Party.get_by_id)]:
...
def tool_check_availability():
    """Check if a listing has any confirmed bookings overlapping the dates."""
    ...
    try:
        ...
        for doc in bookings:
            ...
            if b_in and b_out:
                # Overlap check: ranges overlap if A.start < B.end AND A.end > B.start
...
def tool_create_booking():
    """Create a pending booking on behalf of a caller."""
    args, call = _parse_tool_request()
    ...
    price = listing.get('price') or listing.get('ticketPrice') or 0

    # Rough total: nights × price for homes/experiences; tickets × price for events
    try:
    ...
    total = price * (nights if listing_type in ('home', 'experience') else guests)
...
def tool_transfer_to_host():
    """Return the host's phone number so Retell can perform a transfer_call."""
    args, _ = _parse_tool_request()
    listing_id = args.get('listing_id', '').strip()

    host_phone = None

    for getter in [Home.get_by_id, Experience.get_by_id, Party.get_by_id]:
        result = getter(listing_id)
        if result:
    ...
    return jsonify({
        'phone': None,
        'message': 'Host phone not available — transferring to support.',
    }), 200
...
def _fuzzy_find(name, city):
    """Best-effort search across all categories by title/city substring."""
    for lt, getter in [('home', Home.search), ('experience', None), ('party', None)]:
        if lt == 'home':
            results = getter({})
        elif lt == 'experience':
            results = Experience.get_all_published()
        else:
            results = Party.get_all_published()
        for item in results:
...
def _listing_payload(listing_type, listing):
    ...
    return {
        ...
        'price': listing.get('price') or listing.get('ticketPrice'),
        'price_unit': 'night' if listing_type == 'home' else 'person' if listing_type == 'experience' else 'ticket',
        'description': listing.get('description'),
        'bedrooms': listing.get('bedrooms'),
        'bathrooms': listing.get('bathrooms'),
        'amenities': listing.get('amenities', []),
        'capacity': listing.get('capacity') or listing.get('maxGroupSize'),
        'rating': (listing.get('ratings') or {}).get('average'),
        'event_date': listing.get('dateTime'),
